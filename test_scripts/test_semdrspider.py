...
import pandas as pd
from acmdiff.sem_diff import gen_completediff
...
def eval_prevspost(nl_predir, nl_postdir, sql_dir, pert_names, db_name, db_details):
    ...
    fullfname = outpref + 'sem_nlvsnl_fullstats.csv'
...
def eval_prevssql(nl_predir, sql_dir, pert_names, db_name, db_details):
    ...
    fullfname = outpref + 'sem_nlvssql_fullstats.csv'
    out_df = pd.DataFrame(out_dct)
    out_df.to_csv(fullfname, index=False)
...
def zip_nlq(drspider_dir, nl_predir, nl_postdir, sql_dir, db_name):
    ...
    fullfname = outpref + 'sem_nlvssql_fullstats.csv'
    fullfname2 = outpref + 'sem_nlvsnl_fullstats.csv'
    fullstats_files += [fullfname, fullfname2]
    
    copy_and_zip(res_dirs + stats_dirs, outpref + 'sem_allresults', outpref + 'sem_allresults.zip', additional_files=fullstats_files)

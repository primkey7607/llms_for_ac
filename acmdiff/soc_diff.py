...
from gen_sociomeperts import sociome_schema, sociome_roles
from acmdiff.diff_acms import nlvssql_roles, \
                              nlvsnl_roles, \
                              diff_privs, \
                              privdiff_summary, \
                              viewdiff_summary, \
...
def nlvsnl_views(nldf1, nldf2, nl1_cols, nl2_cols, outdir, outname):
    ...
    for i,v1 in enumerate(views1):
        ...
        if v1 in views2:
            ...
            prompt += 'Which database table or view description from the list most likely describes the same table or view as this phrase? Begin your answer with your chosen description from the list.'
...
def sqlvsnl_views(nl_acmdf, sql_acmdf, lst_acmpath, nl_cols, sql_cols, outdir, outname):
    ...
    for i, db_v in enumerate(db_views):
        ...
        prompt2 += 'Which description from the list does the given query most likely describe? Begin your answer with this description.'
...
def views_are_equal(v1, v2, db_details):
    #the sociome data cannot be put into a database. so, just compare strings.
    #this will be reliable because we know the views are projections
    if v1 == v2:
        return True
    return False
...
def diff_views(acmpath1, acmpath2, outdir, outname, db_details):
    ...
    schema = sociome_schema(db_details['repo'])
